...
from backend.app.routers.data_cleanup import router as cleanup_router
from backend.app.routers.sentiment_cleanup import router as sentiment_cleanup_router
...
async def lifespan(app: FastAPI):
    ...
    # Periodischer Sentiment-Cleanup (täglich um 2 Uhr)
    async def _periodic_sentiment_cleanup():
        while True:
            # Warte bis 2 Uhr morgens
            ...
            try:
                ...
                if db:
                    await db.rpc("cleanup_old_sentiment_data").execute_async()
                    logger.info("Täglicher Sentiment-Cleanup ausgeführt")
            except Exception as e:
                logger.warning(f"Sentiment-Cleanup Fehler: {e}")

    asyncio.create_task(_periodic_sentiment_cleanup())
...
app.include_router(sentiment_cleanup_router)
